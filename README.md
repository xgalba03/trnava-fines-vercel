...
5. Add these variables exactly, selecting **Production** (and Preview if needed):
   ...
   - `SITE_URL`: your deployed Vercel URL, such as `https://trnava-fines.vercel.app`
6. Save the variables, then choose **Deployments -> Redeploy**. Environment variables do not apply to an already-built deployment.

In Supabase, open **Authentication -> URL Configuration** and add your `SITE_URL` to the allowed redirect URLs. The magic link will open the site in Safari on your iPhone.
